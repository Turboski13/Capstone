...
const PlayArea = () => {
  ...
  const getTeamInfo = async (teamId) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://capstone-dk9v.onrender.com/api/teams/${teamId}`,
        {
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${token}`,
      ...
      {isDm && (
        <div className='pa-container'>
          <form
            ...
            onSubmit={(e) => {
              e.preventDefault();
              uploadEnemySheet(teamId, csvFile);
            }}
          >
            <input
              id='csv-input'
            ...
            <input
              id='dm-upload'
              type='checkbox'
              checked={shareAll || false}
              onChange={() =>
                shareAll ? setShareAll(false) : setShareAll(true)
              }
            />
            <button id='dm-upload'>Upload</button>
          </form>
        </div>
      )}
      {!showChars ? (
        <button onClick={() => showUserChars()} className='add-char-btn'>
          Add Your Character To This Adventure
        </button>
      ) : (
        <button onClick={() => setShowChars(false)} className='hide-char-btn'>
      ...
      {showChars &&
        userChars.map((char) => (
          <div key={char.id}>
            ...
            <button onClick={() => addCharToTeam(char.id)} className='join-char-btn'>
              Join With This Character
            </button>
          </div>
        ))}
      <h1 className='pa-team'>Team: {allData.teamName}</h1>
      <TabSwitcher
